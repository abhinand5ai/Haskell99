...
distanceOf info v = maybe Infinity Finite (Map.lookup v (dist info))

dijkstra ::
     forall v c. (Num c, Ord c, Ord v)
  => Graph v c
  -> v
  -> Info v c
dijkstra graph source =
  let loop :: [v] -> Info v c -> Info v c
      loop q info = MkInfo {dist = Map.empty, prev = Map.empty}
   in loop
        (vertices graph)
        (MkInfo {dist = Map.singleton source 0, prev = Map.empty})
